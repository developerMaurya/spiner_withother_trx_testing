...
//         var myModalheader = document.getElementById("myModal1_header");
//         var myModal_result = document.getElementById("myModal1_result");
//         var lottery_result = document.getElementById("lottery1_result");
//         var myModal_result_Period = document.getElementById("myModal1_result_Period");
//         if (firstGame.get == 0) {
...
//       socket.emit('data-server-trx_2', { money: x * money, join, time: Date.now(), change: response.change });
//     },
//   });
...
function showListOrder3(list_orders, x) {
  ...
  $(".foot .right").click(function (e) {
    ...
    $.ajax({
      ...
      dataType: "json",
      success: function (response) {
        console.log("respons data as ...",response)
        alertMessJoin(response.message);
        if (response.status === false) return;
        $("#history-order").prepend(response.data);
        $(".total-box .num span").text("₹ " + response.money + ".00");
  ...
  function showListOrder(list_orders, x) {
    ...
    let htmls = "";
    let result = list_orders.map((list_orders) => {
      const formattedPeriod = `${list_orders.period.toString().slice(0, 3)}**${list_orders.period.toString().slice(-4)}`;
      const formattedHash = `***${list_orders.hash.slice(-4)}`;
      const formattedTime = list_orders.time.split(" ")[1];
      return (htmls += `
        <div class="wrap" style="display: flex; justify-content: space-between; padding: 8px 0;">
                      <div data-v-a9660e98="" class="c-tc item van-row" style="display: flex; flex-wrap: nowrap;">
                          <div data-v-a9660e98="" class="van-col van-col--6" style="padding-right: 10px; padding-left: 10px; color:white; padding-top: 25px;">
                              <div data-v-a9660e98="" class="c-tc goItem">${
                                formattedPeriod
                              }</div>
                          </div>
                          <div data-v-a9660e98="" class="van-col van-col--5" style="padding-right: 10px; padding-left: 10px; color:white;">
                              <img src="/images/a.png" alt="Clickable Image" style="cursor: pointer;" style="width:35%" onclick="openUrl('https://tronscan.org/#/block/${list_orders.block}')" />
                          ${list_orders.block}
                          </div>
                          <div data-v-a9660e98="" class="van-col van-col--5" style="padding-right: 10px; padding-left: 10px; color:white; padding-top: 25px;">
                              <div data-v-a9660e98="" class="c-tc goItem">${
                                formattedTime
                              }</div>
                          </div>
                          <div data-v-a9660e98="" class="van-col van-col--3" style="padding-right: 10px; padding-left: 10px; color:white; padding-top: 25px;">
                              <div data-v-a9660e98="" class="c-tc goItem">${
                                formattedHash
                              }</div>
                          </div>
                          <div data-v-a9660e98="" class="van-col van-col--3" style="padding-right: 10px; padding-left: 10px; color:red; padding-top: 25px;">
                              <div data-v-a9660e98="" class="c-tc goItem">${
                                list_orders.result
                              }</div>
                          </div>
                          <div data-v-a9660e98="" class="van-col van-col--2" style="padding-right: 10px; color:white; padding-top: 25px;">
                              <div data-v-a9660e98="" class="c-tc goItem">${
                                list_orders.bigsmall
                              }</div>
                          </div>
  ...
  window.onload = function () {
    function startCountdown() {
      let countDownDuration = 30 * 1000; // 30 seconds
      ...
      function updateTimer() {
        ...
        if (distance <= 0) {
          ...
          $(".number .item:eq(3)").text(0);
          $(".number .item:eq(4)").text(0);
  
          // Restart the countdown after 30 seconds
          startCountdown(); // Recursive call to restart the countdown
          console.log("Countdown restarted!");
        }
      }
  
      // Run update every 100 milliseconds
