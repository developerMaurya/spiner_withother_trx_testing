...
const giftPage = async (req, res) => {
    return res.render("manage/giftsPage.ejs");
}
const adminChangePassword= async(req, res) => {
    return res.render("manage/changePassword.ejs"); 
}
const addGift= async(req, res) => {
    return res.render("manage/addGift.ejs"); 
}
const adminUpdatePassword = async (req, res) => {
    ...
    try {
        ...
        if (!user || user.length === 0) {
            return res.render('manage/changePassword', { status: 'error', message: 'User not found' });
        }

        // Check if the current password matches
        if (md5(currentPassword) !== user[0].password) {
            return res.render('manage/changePassword', { status: 'error', message: 'Current password is incorrect' });
        }

        // Check if new password matches confirm password
        if (newPassword !== confirmPassword) {
            return res.render('manage/changePassword', { status: 'error', message: 'New passwords do not match' });
        }
        ...
        await connection.query("UPDATE users SET password = ?, plain_password = ? WHERE phone = ?", [hashedPassword, newPassword, phone]);

        return res.render('manage/changePassword', { status: 'success', message: 'Password changed successfully' });
    } catch (error) {
        console.error("Error changing password: ", error);
        return res.render('manage/changePassword', { status: 'error', message: 'Server error' });
    }
};
// Fetch user's money based on phone number


// Update user's money based on the form submission
const updateUserMoney = async (req, res) => {
    const { phone, amount } = req.body;

    try {
        // Fetch the user's current money based on the phone number
        const [user] = await connection.query("SELECT * FROM users WHERE phone = ?", [phone]);

        if (!user || user.length === 0) {
            return res.render('manage/addGift', { status: 'error', message: 'User not found' });
        }

        // Calculate the new money value
        const newAmount = parseFloat(user[0].money) + parseFloat(amount);

        // Update the user's money in the database
        await connection.query("UPDATE users SET money = ? WHERE phone = ?", [newAmount, phone]);

        return res.render('manage/addGift', { status: 'success', message: 'Amount added successfully' });
    } catch (error) {
        console.error("Error updating user money: ", error);
        return res.render('manage/addGift', { status: 'error', message: 'Server error' });
    }
};
