...
const trxwingoPage3 = async (req, res) => {
    return res.render("bet/wingo/trx30sec.ejs");
}
...
const betWinGo = async (req, res) => {
    ...
    let period = (parseInt(winGoNow[0].period, 10) + 1).toString();
...
// console.log("pass in add type.....")
        ...
        status = ?,
        time = ?`;
// console.log("result is .here ....",Number(period) + 1, 0, join, 0, timeNow)
...
console.log("typeid handle wingo is...",typeid)
    let game = '';
    if (typeid == 1) game = 'wingo';
    if (typeid == 3) game = 'wingo3';
    if (typeid == 5) game = 'wingo5';
...
//
const [periodDatafromDatabase] = await connection.execute('SELECT period FROM trx WHERE type = 1 ORDER BY id DESC LIMIT 1', []);
            let periodfrom = periodDatafromDatabase[0].period;
                // Convert the period to a number and add 1
                const newPeriodfromdatabase = Number(periodfrom);
    ...
    if (check >= 0) {
        ...
        time = ?,
        type=?`;
        await connection.execute(sql, [id_product, userInfo.phone, userInfo.code, userInfo.invite, newPeriodid, userInfo.level, total, x, fee, 0, gameJoin, join, 0, checkTime, timeNow,typeid]);
        await connection.execute('UPDATE `users` SET `money` = `money` - ? WHERE `token` = ? ', [money * x, auth]);
        const [users] = await connection.query('SELECT `money`, `level` FROM users WHERE token = ? AND veri = 1  LIMIT 1 ', [auth]);
        await rosesPlus(auth, money * x);
        return res.status(200).json({
...
const trxhandlingWinGo1P = async (typeid,resultData) => {
    // console.log("ab.....")
...
// console.log("wingonow data is ...",winGoNow)
    // update ket qua
    // await connection.execute(`UPDATE trxminutes_1 SET result = ? WHERE status = 0 AND game = '${game}'`, Number([resultData]));
    await connection.execute(`UPDATE trxminutes_1 SET result = ? WHERE status = 0 AND game = ?`, [Number(resultData), game]);

    // let result = Number([resultData]);
    let result = Number(resultData);
    switch (result) {
        case 0:
    ...
    for (let i = 0; i < order.length; i++) {
        ...
        const [users] = await connection.execute('SELECT `money` FROM `users` WHERE `phone` = ?', [phone]);
        let totals = parseFloat(users[0].money) + parseFloat(nhan_duoc);
        console.log("get mondy as ....",parseFloat(nhan_duoc))
