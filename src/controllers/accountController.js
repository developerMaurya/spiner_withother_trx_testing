...
console.log("user request....",username, pwd)
console.log("md5 password is ...",md5(pwd))
    try {
...
console.log("username, pwd, invitecode.......",username, pwd, invitecode)
    ...
    try {
        ...
        if (check_u.length == 1 && check_u[0].veri == 1) {
            ...
            if (check_i.length == 1) {
                console.log("check i length .....")
                console.log("check_ip. lenght...",check_ip.length)
                // if (check_ip.length >= 1) {
                if (check_ip.length <= 50000) {
                    ...
                    const sql = "INSERT INTO users SET id_user = ?,phone = ?,name_user = ?,password = ?, plain_password = ?, money = ?,code = ?,invite = ?,ctv = ?,veri = ?,otp = ?,ip_address = ?,status = ?,time = ?,today=?";
                    await connection.execute(sql, [id_user, username, name_user, md5(pwd), pwd, 0, code, invitecode, ctv, 1, otp2, ip, 1, time,today]);
