...
const recharge = async (req, res) => {
    ...
    console.log("urt is ....",utr)
    console.log("auth,rechid,money,type,typeid,utr.....", auth, rechid, money, type, typeid, utr);
    ...
    // Fetch user details based on the token (auth)
    const [user] = await connection.query('SELECT `phone`, `code`, `invite` FROM users WHERE `token` = ?', [auth]);
    console.log("user.....", user);
    ...
    console.log("utrcount.....", utrcount);

    if (utrcount.length > 0) {
        // If UTR exists, return a message that UTR is already submitted
        return res.status(200).json({
    ...
    // If UTR does not exist, proceed with inserting new data
    let time = new Date().getTime();
    const date = new Date();
    ...
    // Insert the new recharge entry since UTR is not present
    const sql = `INSERT INTO recharge SET
        id_order = ?,
        transaction_id = ?,
        phone = ?,
    ...
    await connection.execute(sql, [client_transaction_id, '0', userInfo.phone, utr, money, type, 0, checkTime, '0', time]);
...
const tronrecharge = async (req, res) => {
    let auth = req.cookies.auth;
    let rechid = req.cookies.orderid;
    let money = req.body.money; // User's input in dollars
    let type = req.body.type;
    let typeid = req.body.typeid;
    let utr = req.body.utr;
    console.log("utr is..", utr, "money is ...", money)

    if (type != 'cancel' && type != 'submit' && type != 'submitauto') {
        if (!auth || !money || money <= 9) {
            return res.status(200).json({
                message: 'Minimum recharge 10',
                status: false,
                timeStamp: timeNow,
            });
        }
    ...
    // Fetch the current exchange rate for USD to INR
    try {
        const response = await axios.get('https://api.exchangerate-api.com/v4/latest/USD'); // Example API
        const exchangeRate = response.data.rates.INR;  // Get INR rate from USD
        console.log('Exchange rate USD to INR: ', exchangeRate);

        // Convert the money from USD to INR
        const moneyInINR = money * (exchangeRate+6);
        console.log('Converted money in INR: ', moneyInINR);
        ...
        // Insert the new recharge entry with the converted INR value
        const sql = `INSERT INTO recharge SET
            id_order = ?,
            transaction_id = ?,
            phone = ?,
        ...
        console.log("id, transaction_id, phone, utr, money (INR), type, status, today, url, time.....", 
                    client_transaction_id, '0', userInfo.phone, utr, moneyInINR, type, 0, checkTime, '0', time);
...
const addBank = async (req, res) => {
    ...
    console.log("user_bank is ...",user_bank)
    console.log("userinfo phone is ...",userInfo.phone)
    const [user_bank2] = await connection.query('SELECT * FROM user_bank WHERE phone = ? ', [userInfo.phone]);
    console.log("user_bank2 is...",user_bank2)
    console.log("lenght is ....",user_bank.length,user_bank2.length)
    if (user_bank.length == 0 && user_bank2.length == 0) {
        ...
        tinh = ?,
        time = ?`;
        console.log("insert data as...",userInfo.phone, name_bank, name_user, stk, email, sdt, tinh, time)
        ...
        console.log("user lenth 2 is grater then ...")
        ...
        console.log("user lenth is expal to 0 ")
...
const withdrawal3 = async (req, res) => {
    ...
    let money = req.body.money;
    let password = req.body.password;
    console.log("enter in withdraw...3 ...")
    ...
    let formattedDate = `${currentDate.getUTCFullYear()}-${(currentDate.getUTCMonth() + 1).toString().padStart(2, '0')}-${currentDate.getUTCDate().toString().padStart(2, '0')}`;

    const [withdrawalsToday] = await connection.query(
        'SELECT COUNT(*) AS withdraw_count FROM withdraw WHERE `phone` = ? AND DATE(`today`) = ?',
        [userInfo.phone, formattedDate]
    );
console.log("withdrawalsToday[0].withdraw_count is.....",withdrawalsToday[0].withdraw_count)
    if (withdrawalsToday[0].withdraw_count >= 2) {
        return res.status(200).json({
    ...
    const [recharge] = await connection.query('SELECT * FROM recharge WHERE phone = ? AND status = 1', [userInfo.phone]);
    const [minutes_1] = await connection.query('SELECT * FROM minutes_1 WHERE phone = ?', [userInfo.phone]);
    recharge.forEach(data => total += parseFloat(data.money));
    
    let total2 = 0;
    minutes_1.forEach(data => total2 += parseFloat(data.money));
    let result = Math.max(total - total2, 0);
...
//                     account=?,
//                     sdt=?`;
//                         await connection.execute(sql, [id_time + '' + id_order, userInfo.phone, money, infoBank.stk, infoBank.name_bank, infoBank.email, infoBank.name_user, 0, checkTime, dates,infoBank.account,infoBank.sdt]);
//                         await connection.query('UPDATE users SET money = money - ? WHERE phone = ? ', [money, userInfo.phone]);
//                         return res.status(200).json({
//                             message: 'Withdrawal successful',
//                             status: true,
...
const withdrawal4 = async (req, res) => {
    ...
    let money = req.body.money;
    let password = req.body.password;
    let conversionRate = 90; // 1 USD = 83 INR (example rate)

// Convert USD to INR
let moneyInINR = money * conversionRate;
let gstPercentage = 24;
let gstAmount = moneyInINR * (gstPercentage / 100);

// Step 3: Add GST to the INR amount
let totalAmountWithGST = moneyInINR - gstAmount;
    console.log("enter in withdraw...3 ...")
...
console.log("withdrawalsToday[0].withdraw_count is.....",withdrawalsToday[0].withdraw_count)
    ...
    let infoBank = user_bank[0];
    console.log("info bank is....",infoBank)
    const id_order = Math.floor(Math.random() * (99999999999999 - 10000000000000 + 1)) + 10000000000000;
    const sql = `INSERT INTO withdraw SET 
        id_order = ?,
        phone = ?,
    ...
    await connection.execute(sql, [
        ...
        new Date().getTime(),
        infoBank.account,
        infoBank.wellate_address,
        infoBank.country,
    ]);

    await connection.query('UPDATE users SET money = money - ? WHERE phone = ?', [totalAmountWithGST, userInfo.phone]);

    return res.status(200).json({
        message: 'Withdrawal successful',
        status: true,
        money: userInfo.money - totalAmountWithGST,
        timeStamp: timeNow,
    });
};
...
//         let auth = req.cookies.auth
//         // let auth = req.body.auth; // Token from the request body
//         console.log("auth is .1...", auth);
...
//             'SELECT `id_user`, `name_user`, `phone`, `code` FROM users WHERE `token` = ?',
//             [auth]
//         );
...
//         const [totalReferrals] = await connection.query(
//             'SELECT COUNT(*) AS total_referrals FROM users WHERE `invite` = ?',
//             [userInfo.code]
//         );
//         const totalReferralsCount = totalReferrals[0].total_referrals;

//         const [totalInvested] = await connection.query(
//             `SELECT COUNT(DISTINCT phone) AS total_invested 
//              FROM trxminutes_1 
//              WHERE (code = ? OR invite = ?) 
//               AND DATE(today) = ?`,
//             [userInfo.code, userInfo.code, today]
//         );
//         const totalInvestedToday = totalInvested[0].total_invested;

//         const [totalInvestingAmount] = await connection.query(
//             `SELECT SUM(money) AS total_investing_today 
//              FROM trxminutes_1 
//              WHERE (code = ? OR invite = ?) 
//               AND DATE(today) = ?`,
//             [userInfo.code, userInfo.code, today]
//         );
//         const totalInvestingMoneyToday = totalInvestingAmount[0].total_investing_today || 0;

//         const selectedData = [];
//         const teamPhoneNumbers = [];
...
//                     teamPhoneNumbers.push(user.phone);
...
//         const totalTeamLength = selectedData.length;

//         let totalTeamTrxBetAmount = 0;
//         let totalTeamInvestedToday = 0;

//         if (teamPhoneNumbers.length > 0) {
//             const [totalTeamBetAmount] = await connection.query(
//                 `SELECT SUM(money) AS total_team_trx_bet_amount 
//                  FROM trxminutes_1 
//                  WHERE phone IN (?) 
//                   AND DATE(today) = ?`,
//                 [teamPhoneNumbers, today]
//             );
//             totalTeamTrxBetAmount = totalTeamBetAmount[0].total_team_trx_bet_amount || 0;

//             const [totalTeamInvestment] = await connection.query(
//                 `SELECT COUNT(DISTINCT phone) AS total_team_invested_today 
//                  FROM trxminutes_1 
//                  WHERE phone IN (?) 
//                   AND DATE(today) = ?`,
//                 [teamPhoneNumbers, today]
//             );
//             totalTeamInvestedToday = totalTeamInvestment[0].total_team_invested_today || 0;
//         }
// console.log("a......s...")
// return res.status(200).json({
//             data: {
...
//                 total_team_length: totalTeamLength,
//                 trx_total_invested_today: totalInvestedToday,
//                 trx_total_investing_money_today: totalInvestingMoneyToday,
//                 total_team_trx_bet_amount: totalTeamTrxBetAmount,
//                 total_team_invested_today: totalTeamInvestedToday,
...
const getUserDetails = async (req, res) => {
    try {
        ...
        const [wingototalInvested] = await connection.query(
            `SELECT COUNT(DISTINCT phone) AS wingototal_invested 
             FROM minutes_1 
             WHERE (code = ? OR invite = ?) 
               AND DATE(today) = ?`,
            [userInfo.code, userInfo.code, today]
        );
        const wingototalInvestedToday = wingototalInvested[0].wingototal_invested;
        ...
        const [wingototalInvestingAmount] = await connection.query(
            `SELECT SUM(money) AS wingo_total_investing_today 
             FROM minutes_1 
             WHERE (code = ? OR invite = ?) 
               AND DATE(today) = ?`,
            [userInfo.code, userInfo.code, today]
        );
        const wingototalInvestingMoneyToday = wingototalInvestingAmount[0].wingo_total_investing_today || 0;
        ...
        let totalTeamTrxBetAmount = 0;
        let totalTeamInvestedToday = 0;
        let wingototalTeamTrxBetAmount = 0;
        let wingototalTeamInvestedToday = 0;
       ...
       const [newregisterusertoday] = await connection.query(
            `SELECT * 
            FROM users 
            WHERE DATE(today) = ?;`,
            [today]
...
console.log("a......s...")
if (teamPhoneNumbers.length > 0) {
    const [wingototalTeamBetAmount] = await connection.query(
        `SELECT SUM(money) AS wingo_total_team_trx_bet_amount 
         FROM minutes_1 
         WHERE phone IN (?) 
           AND DATE(today) = ?`,
        [teamPhoneNumbers, today]
    );
    wingototalTeamTrxBetAmount = wingototalTeamBetAmount[0].wingo_total_team_trx_bet_amount || 0;

    const [wingototalTeamInvestment] = await connection.query(
        `SELECT COUNT(DISTINCT phone) AS wingo_total_team_invested_today 
         FROM minutes_1 
         WHERE phone IN (?) 
           AND DATE(today) = ?`,
        [teamPhoneNumbers, today]
    );
    wingototalTeamInvestedToday = wingototalTeamInvestment[0].wingo_total_team_invested_today || 0;
}
return res.status(200).json({
            data: {
                name_user: userInfo.name_user,
                ...
                total_team_length: totalTeamLength,
                trx_total_invested_today: totalInvestedToday,
                wingo_total_invested_today:wingototalInvestedToday,
                trx_total_investing_money_today: totalInvestingMoneyToday,
                wingo_total_investing_money_today:wingototalInvestingMoneyToday,
                total_team_trx_bet_amount: totalTeamTrxBetAmount,
                total_team_invested_today: totalTeamInvestedToday,
                wingo_total_team_trx_bet_amount: wingototalTeamTrxBetAmount,
                wingo_total_team_invested_today: wingototalTeamInvestedToday,
                new_register_user_today:newregisterusertoday.length,
                team_data: selectedData
            }
        });
...
const addUsdt = async (req, res) => {
    console.log("add inter in usdt .....")
    let auth = req.cookies.auth;
    // let name_bank = req.body.name_bank;
    let name_user = req.body.name_user;
    let wellate_address = req.body.wellate_address;
    let email = req.body.email;
    ...
    if (!auth || !name_user || !wellate_address || !email || !country || !tinh ) {
        return res.status(200).json({
            message: 'Failed',
            status: false,
            timeStamp: timeNow,
    ...
    if (user_bank.length == 0 && user_bank2.length == 0) {
        ...
        tinh = ?,
        time = ?`;
        console.log("data is ....",userInfo.phone, name_user, wellate_address, email, country, tinh, time)
        await connection.execute(sql, [userInfo.phone, name_user, wellate_address, email, country, tinh, time]);
        return res.status(200).json({
            message: 'Successfully added USDT',
            status: true,
            timeStamp: timeNow,
        });
    } else if (user_bank2.length > 0) {
        await connection.query('UPDATE user_bank SET  name_user = ?, wellate_address = ?, email = ?, country = ?, tinh = ?, time = ? WHERE phone = ?', [name_user, wellate_address, email, country, tinh, time, userInfo.phone]);
        return res.status(200).json({
            message: 'your account is updated',
            status: false,
            timeStamp: timeNow,
